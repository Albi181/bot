...
func (b *Client) SubscribeTicker(symbol Symbol, interval time.Duration) chan OrderBookTickerStreamResult {
	...
	// Set the ping handler to automatically reply with pong
	conn.SetPingHandler(func(message string) error {
		err := conn.WriteControl(websocket.PongMessage, []byte(message), time.Now().Add(time.Second))
		if err != nil {
			slog.Error("Handle pong sending error", "error", err)
			// Handle pong sending error
		}
		return nil
	})

	orders := make(chan OrderBookTickerStreamResult)
	go func() {
		now := time.Now()
		for {
